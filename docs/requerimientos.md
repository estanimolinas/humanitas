...
La fuente de verdad es [humanitas_requerimiento_mvp.md](humanitas_requerimiento_mvp.md). Este documento lo cruza con el código: para cada requerimiento P0 dice en qué estado está, dónde está hecho y qué test lo comprueba. También lista las decisiones del usuario que cambiaron el requerimiento original.

Estado al 19/09/2026. 143 tests en `tests/` (`npm test`, contra Supabase local).
...
| R14 | Verificación presencial | ❌ Sacado por decisión del usuario (18/09): sin puntos de alta ni instituciones | — | — |
| R15 | Registro de eventos mínimos | ⏳ Pendiente: la tabla existe; dejado para después del [interno] (decisión 17/09) | tabla `eventos` | — |
...
Ninguno está hecho. Por decisión del usuario, el MVP es para validar con un [interno]:

- **R20 y R21** (avisos por WhatsApp): después del [interno].
...
## Decisiones del usuario que cambian el requerimiento

| Fecha | Decisión | Cambia |
|---|---|---|
| 17/09/2026 | En la app hay solo personas: sin pantallas de operador ni de referente. Esas tareas se hacen con scripts del equipo. | 4.3, R24 |
| 17/09/2026 | El objetivo es un MVP para un [interno] y el [interno]: la app más simple posible. | Alcance |
...
El detalle y los motivos de cada decisión están en [CLAUDE.md](../CLAUDE.md).
...
4. **R15 eventos y métricas:** después del [interno].
