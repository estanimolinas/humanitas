...
export function validarPaso1(c: CamposPublicacion): ErroresPublicacion {
  ...
  if (c.tipo !== "ofrezco" && c.tipo !== "necesito") errores.tipo = "Todavía falta elegir una de las dos opciones.";
  else if (c.tipo === "ofrezco" && c.subtipo !== "servicio" && c.subtipo !== "producto") {
    errores.subtipo = "Todavía falta indicar si es un servicio o un producto.";
  }
  return errores;
}
...
export function validarPaso2(c: CamposPublicacion, rubroEsOtros = false): ErroresPublicacion {
  ...
  if (!Number.isInteger(rubro) || rubro <= 0) errores.rubroId = "Todavía falta elegir un rubro.";
  if (rubroEsOtros && limpio(c.rubroOtroTexto).length === 0) {
    errores.rubroOtroTexto = "Nos falta saber de qué se trata.";
  } else if (limpio(c.rubroOtroTexto).length > 80) {
    errores.rubroOtroTexto = "El texto es un poco largo. ¿Lo acortamos?";
  }

  const titulo = limpio(c.titulo);
  if (titulo.length === 0) errores.titulo = "Nos falta un título corto.";
  ...
  if (limpio(c.precioTexto).length > 80) errores.precioTexto = "El texto es un poco largo. ¿Lo acortamos?";
  if (limpio(c.aliasPago).length > 80) errores.aliasPago = "El texto es un poco largo. ¿Lo acortamos?";
  return errores;
}
...
export function validarPaso3(c: CamposPublicacion): ErroresPublicacion {
  ...
  if (c.zonaId && c.zonaOtroTexto) {
    errores.zonaOtroTexto = "Podés elegir un barrio de la lista o escribir otro, pero no los dos a la vez.";
  }
  if (limpio(c.zonaOtroTexto).length > 80) errores.zonaOtroTexto = "El texto es un poco largo. ¿Lo acortamos?";
  return errores;
}
